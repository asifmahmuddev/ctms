...
def profile_image_path(instance, _filename):
    ...
    Each account keeps a single image in its own directory, so the uploaded name is replaced by a
    stable one. Django passes that name positionally, so the unused parameter keeps its place.
    """
...
class Account(AbstractBaseUser):
    ...
    # Administrators hold every permission. No per-object or per-app rules are defined, so the
    # arguments Django passes positionally are unused.
